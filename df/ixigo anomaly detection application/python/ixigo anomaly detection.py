...
class ExecutionHandler(BaseExecutionHandler):
    ...
    def execute(self, df: pd.DataFrame, observation_period: int, forecast_period: int, timestamp_column, target_column,
                threshold_percentage, covariate_columns=[]):
        ...
        # df = df[[target_column,timestamp_column]].groupby(timestamp_column).mean()
        ...
        if len(covariate_columns) == 0:
            ...
            trimmed_df.rename(columns={target_column: 'Actual Value', 'trend': 'Trend Value'}, inplace=True)
            trimmed_df = trimmed_df[['Actual Value', 'Trend Value', 'Absolute Difference From Trend In %',
        ...
        # -----When target column depend on covariates
            ...
            trend = model2.predict(trend_df)['yhat']

            trimmed_df = trimmed_df[-len(trend):].reset_index().drop('index', axis=1)
            ...
            trimmed_df['Is Significant (Threshold Is ' + str(threshold_percentage) + '%)'] = trimmed_df[
                                                                                                 'Absolute Difference From Trend In %'] > threshold_percentage
            trimmed_df.rename(columns={'y': 'Actual Value', 'trend': 'Trend Value'}, inplace=True)
            trimmed_df = trimmed_df[['Actual Value', 'Trend Value', 'Absolute Difference From Trend In %',
                                     'Is Significant (Threshold Is ' + str(threshold_percentage) + '%)']]
        ...
        return trimmed_df[-100:], output_df
