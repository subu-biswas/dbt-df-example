...
class ExecutionHandler(BaseExecutionHandler):
    def execute(self, segment_data, CustomerID, segment_column='segment', descriptor_data=None, method='Logistic Regression'):
        try:
            ...
            if len(CustomerID) > 0:
                ...
                CustomerID = ['ID']
                temp_data[descriptor_data.columns] = descriptor_data[descriptor_data.columns]
            descriptor_data = self.numeric_column_scale(descriptor_data)
            descriptor_data = self.categorical_column_encode(descriptor_data)
            ...
            if method == 'Logistic Regression':
                ...
                print(cm)
                print('')
                report = classification_report(y_train, model.predict(X_train))  # Model Validation Report
                ...
                model_prediction = pd.concat([temp_data[CustomerID], model_prediction], axis=1)
